...
async function observerCallback() {
	...
	if (isOnReader && !wasOnReader) {
		...
		root.style.setProperty(
			'--ttu-whispersync-yatsu-surface',
			'var(--surface-strong-color, rgba(255, 255, 255, 0.94))',
		);
		root.style.setProperty(
			'--ttu-whispersync-yatsu-surface-soft',
			'var(--surface-color, rgba(255, 255, 255, 0.78))',
		);
		root.style.setProperty('--ttu-whispersync-yatsu-border', 'var(--border-color, rgba(148, 163, 184, 0.26))');
		root.style.setProperty('--ttu-whispersync-yatsu-accent', 'var(--accent-color, #0f766e)');
